...
export async function runCheck(accountId) {
  ...
  // Check every account's key in parallel — sequential awaits here meant total
  // refresh time was the SUM of every account's API round-trip; parallel makes
  // it as fast as the single slowest one.
  await Promise.all(toCheck.map(async (acc) => {

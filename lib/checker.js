  ...
  try {
    const res = await fetch(CAPI_URL, {
      ...
      body: JSON.stringify({
        ...
        // Explicitly disable extended thinking. Some Aerolink plans otherwise
        // force a thinking budget and reject the request unless
        // max_tokens > budget_tokens — which made valid keys show a false
        // "Error". Disabling thinking removes that constraint entirely, so a
        // tiny max_tokens works on every plan (cheapest possible probe).
        max_tokens: 16,
